...
if uploaded_file:
    try:
        ...
        if st.button("Generate Monitoring Report"):
            ...
            # Export Report (HTML and PDF with reportlab)
            ...
            if st.button("Export Report as PDF"):
                # Prepare PDF content
                pdf_file = "monitoring_report.pdf"
                ...
                # Header
                elements.append(Paragraph("Monitoring Report (Red Fleet Reservoir)", title_style))
                elements.append(Paragraph(f"Water: Red Fleet Reservoir", normal_style))
                elements.append(Paragraph(
                    f"Date(s): {pd.to_datetime(df['Date'].min()).strftime('%m/%d/%Y')}-{pd.to_datetime(df['Date'].max()).strftime('%m/%d/%Y')}",
                    normal_style))
                elements.append(
                    Paragraph("Stocking Strategy: Not specified (assumed predator management and forage enhancement)",
                              normal_style))
                elements.append(
                    Paragraph(f"Target Species: {', '.join(abundance_df['Target Species'].unique())}", normal_style))
                elements.append(Paragraph(
                    f"Methods Description: Electrofishing conducted over multiple transects, total effort {effort} hours.",
                    normal_style))

                # Gear/Effort Table
                gear_data = [["Gear Type", "Effort", "Water Temp (°F)", "Additional Data"],
                             ["Electrofishing", f"{effort} Hr.", f"{df[temp_column].mean().round(1)}",
                              "Diet (some species), length, weight"]]
                gear_table = Table(gear_data)
                ...
                elements.append(gear_table)

                # Abundance Table
                abundance_data = [abundance_df.columns.tolist()] + abundance_df.round(2).values.tolist()
                abundance_table = Table(abundance_data)
                ...
                elements.append(Paragraph("Abundance, Condition, and Proportional Size Distribution of Target Species",
                                          title_style))
                elements.append(abundance_table)

                # Catch Summary Table
                catch_data = [catch_df.columns.tolist()] + catch_df.round(2).values.tolist()
                catch_table = Table(catch_data)
                ...
                elements.append(Paragraph("Catch Summary", title_style))
                elements.append(catch_table)

                # Water Quality (if available)
                if 'Water Quality' in locals():
                    water_data = [["Conductivity (µS)", "pH", "Temp (°F)"]] + [water_quality.round(2).values.tolist()]
                    ...
                    elements.append(Paragraph("Water Quality", title_style))
                    elements.append(water_table)

                # Comments (if available)
                if comments:
                    elements.append(Paragraph("Comments", title_style))
                    elements.append(Paragraph(comments, normal_style))

                # Build PDF
                doc.build(elements)
                st.download_button("Download PDF Report", open("monitoring_report.pdf", "rb").read(),
                                   "monitoring_report.pdf", "application/pdf")
